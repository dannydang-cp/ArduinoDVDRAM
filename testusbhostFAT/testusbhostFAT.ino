        ...
        if(usbon) {
                ...
                if(fatready) {
                        ...
                        if(!notified) {
                                ...
                                // Change code here to test different files

                                printf_P(PSTR("\r\nOpen an existing file (test.txt).\r\n"));
                                rc = f_open(&My_File_Object_x, "0:/test.txt", FA_READ);

                                // printf_P(PSTR("\r\nOpen an existing file (testprint.stl).\r\n"));
                                // rc = f_open(&My_File_Object_x, "0:/testprint.stl", FA_READ);

                                // if(rc) printf_P(PSTR("Error %i, message.txt not found.\r\n"), rc);
                                if(rc) printf_P(PSTR("Error %i, testprint.stl not found.\r\n"), rc);
